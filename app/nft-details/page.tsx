...
const AssetDetails = () => {
  ...
  const checkout = async () => {
    try {
      ...
      toast.error("Error occured while purchasing.");
    }
  };
    ...
    <div className="relative flex justify-center md:flex-col min-h-screen">
      ...
      <div className="flex-1 justify-start sm:px-4 p-12 sm:pb-4">
        ...
        <div className="flex flex-row sm:flex-col mt-10">
          {currentAddress?.toLowerCase() === nft.seller.toLowerCase() ? (
            <p className="font-poppins dark:text-white text-nft-black-1 font-normal text-base border border-gray p-2">
              You cannot buy your own NFT
            </p>
          ) : currentAddress?.toLowerCase() === nft.owner.toLowerCase() ? (
            <Button
              btnName="List on Marketplace"
              btnType="primary"
              classStyles="mr-5 sm:mr-0 sm:mb-5 rounded-xl"
              handleClick={() => router.push(`/resell-nft?id=${nft.tokenId}`)}
            />
          ) : (
            <Button
